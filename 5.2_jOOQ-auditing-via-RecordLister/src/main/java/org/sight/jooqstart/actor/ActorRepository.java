...
import org.sight.jooqstart.config.RecordAuditListenerGenerator;
...
public class ActorRepository {
    ...
    public ActorRepository(Configuration originConfig) {
        Configuration config = originConfig.derive();
        config.set(actorRecordAuditListener());
        this.actorDao = new ActorDao(config);
     ...
     * @return
     */
    private RecordListener actorRecordAuditListener() {
        Consumer<ActorRecord> audit = record -> record.setLastUpdate(LocalDateTime.now());
        return RecordAuditListenerGenerator.<ActorRecord>builder()
                .recordClass(ActorRecord.class)
                .insertStart(audit)
                .updateStart(audit)
                .storeStart(audit)
                .build().generate();
    ...
    public int updateWithRecord(Long actorId, ActorUpdateRequest request) {
        ...
        // RecordListener는 record update 또는 DAO를 통한 update에서만 동작
