...
public class ActorRepository {
    ...
    public List<ActorFilmography> findActorFilmographyByFullNameWithOrAnd(ActorFilmographySearchOption searchOption) {
        final JFilmActor FILM_ACTOR = JFilmActor.FILM_ACTOR;
        final JFilm FILM = JFilm.FILM;
        ...
        Map<Actor, List<Film>> actorListMap = dslContext.select(
                ...
                .where(
                        // 배우 full name like 검색
                        containsIfNotBlank(ACTOR.FIRST_NAME, searchOption.getActorName()).or(
                                containsIfNotBlank(ACTOR.LAST_NAME, searchOption.getActorName())
                        ).and(
                                // 영화 제목 like 검색
                                containsIfNotBlank(FILM.TITLE, searchOption.getFilmTitle())
                        )
    ...
    public List<Actor> fetchActorByFirstNameAndLastName(String firstName, String lastName) {
        return dslContext.selectFrom(ACTOR)
                .where(
                        ACTOR.FIRST_NAME.eq(firstName),
